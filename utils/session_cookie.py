...
Streamlit ties st.session_state to a single browser tab -- a new tab gets a
brand-new, empty session. This layer adds a signed, sliding-expiry token in
a browser cookie (shared across every tab of the same browser) so opening a
new tab doesn't re-prompt for login, and ~1 hour of inactivity naturally
logs the person out.

The cookie payload is just the username -- role/region are never trusted
from it; callers re-fetch those fresh from the database on every restore, so
a demoted or deleted account takes effect immediately rather than waiting
...
COOKIE_NAME = "load_outage_session"
...
def _serializer() -> URLSafeTimedSerializer:
    ...
    return URLSafeTimedSerializer(secret, salt="load-outage-session")
...
def _controller() -> CookieController:
    ...
    cache_key = "_load_outage_cookie_controller"
    if cache_key not in st.session_state:
        st.session_state[cache_key] = CookieController(key="load_outage_cookies")

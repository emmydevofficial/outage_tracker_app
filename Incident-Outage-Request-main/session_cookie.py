...
Deliberately parallel to (but independent from) the other two apps' own
session_cookie.py modules -- same pattern, own cookie name and signing
secret, since this app's login is fully separate from theirs.
"""
import os
...
COOKIE_NAME = "incident_outage_session"
...
def _serializer() -> URLSafeTimedSerializer:
    ...
    return URLSafeTimedSerializer(secret, salt="incident-outage-session")
...
def _controller() -> CookieController:
    ...
    cache_key = "_incident_outage_cookie_controller"
    if cache_key not in st.session_state:
        st.session_state[cache_key] = CookieController(key="incident_outage_cookies")

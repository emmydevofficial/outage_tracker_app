"""TCN design system, copied from outage_tracker-main/utils/branding.py.

Copied rather than imported across folders -- this app is an independent
deployment (own DB, own users, own port), same convention already used by
the other two apps here. The user explicitly asked this app look like the
33kV Load & Outage Analytics app rather than the TCN 330/132kV Outage
Manager app, so this file (and its assets/ folder) is that app's design
system, adapted only to point at this app's own BASE_DIR.
"""
import base64
import time
